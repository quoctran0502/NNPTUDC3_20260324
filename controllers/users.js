...
let { sendUserImportEmail } = require('../utils/sendMail');
...
module.exports = {
    ImportUsersFromFile: async function (filePath) {
        ...
        // Assuming row 1 is header: username, email
        // Start from row 2
        for (let i = 2; i <= worksheet.rowCount; i++) {
            ...
            try {
                ...
                let newUser = new userModel({
                    username: username,
                    password: password, // Schema pre-save hook will hash this
                    email: email,
                    role: userRole._id,
                ...
                // Send email
                await sendUserImportEmail(email, username, password);
                
                results.success.push({ username, email });
            } catch (error) {
                results.errors.push({ username, email, error: error.message });
            }
        }
